...
def generate(input_file, output_folder, zoom_level, resize_width, quiet):
	...
	for i, tile in enumerate(tiles):
		col = i % num_tiles
		f = i / num_tiles
		dst = output_folder + '/' + str(zoom_level) + '/' + str(col) + '/' + str(int(f)) + '.png'
...
if __name__ == '__main__':
	...
	p.add_argument('input_file', help='large image file to split (JPG, PNG, or TIFF)', default='CityPark.png')
	p.add_argument('zoom_level', help='zoom level(s) to generate (0 to 18); either integer or range (ex: 2-6)', default='0-4')
	p.add_argument('output_folder', help='folder name to write tiles to (will be created if does not exist)', default='maps')
